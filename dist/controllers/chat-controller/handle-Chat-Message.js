...
exports.handle_chat_message = void 0;
...
const summarization_Utils_1 = require("../../lib/summarization-Utils");
const throw_error_1 = require("../../utils/throw-error");
// Zod Schema for validation
const handle_chat_message_schema = zod_1.z.object({
    body: zod_1.z.object({
        message: zod_1.z.string().min(1, "Message is required"),
        conversation_id: zod_1.z.string().min(1, "Conversation ID is required"),
        modelName: zod_1.z.string().optional(),
    }),
});
const handle_chat_message = async (req, res) => {
    ...
    const recent_user_messages = await chat_Model_1.default.aggregate([
        { $match: { user: user_id } },
        { $unwind: '$messages' },
    ...
    if (recent_count >= 5) {
        const last_message = await chat_Model_1.default.aggregate([
            { $match: { user: user_id } },
            { $unwind: '$messages' },
            { $match: { 'messages.sender': 'user' } },
            { $sort: { 'messages.timestamp': -1 } },
        ...
        if (last_message.length > 0) {
            ...
            if (time_diff < 10000) { // 10 seconds
                throw new throw_error_1.TAppError("Please wait a moment before sending another message.", 429);
            }
        }
    }
    ...
    if (!openai_key && !mistral_key) {
        throw new throw_error_1.TAppError('No AI API keys configured. Please set OPENAI_API_KEY or MISTRAL_API_KEY', 500);
    }
    const crm_context = await (0, utils_1.get_crm_context)(user_id);
    let conversation = await chat_Model_1.default.findOne({ conversationId: conversation_id, user: user_id });
    if (!conversation) {
        conversation = await chat_Model_1.default.create({
    ...
    const user_msg = {
        sender: types_1.SenderType.USER,
        message,
        timestamp: new Date(),
    };
    ...
    const ai_provider = {
        generateText: async (options) => {
            const result = await (0, ai_Providers_1.stream_text_with_fallback)({
                options: {
                ...
                fallbackModels: (0, ai_Providers_1.get_recommended_fallbacks)(normalized_model_name)
            });
    ...
    if (conversation.summary) {
        context_tokens += (0, summarization_Utils_1.estimate_tokens)(conversation.summary);
    }
    context_tokens += (0, summarization_Utils_1.estimate_messages_tokens)({ messages: recent_messages });
    // Proactive summarization: Check if the optimized context would exceed threshold
    if (context_tokens > summarization_Utils_1.DEFAULT_TOKEN_CONFIG.tokenThreshold) {
        ...
        if (messages_to_summarize.length > 0) {
            const summarized_context = await (0, summarization_Utils_1.process_message_context)({
                allMessages: messages_to_summarize,
                config: summarization_Utils_1.DEFAULT_TOKEN_CONFIG,
    ...
    const system_prompt = (0, utils_1.create_system_prompt)(crm_context);
    ...
    formatted_messages.push(...final_recent_messages.map((msg) => ({
        role: msg.sender === types_1.SenderType.USER ? 'user' : 'assistant',
        content: msg.message,
    })));
    // Log the final context token count
    const context_text = formatted_messages.map(m => m.content).join('\n');
    const final_context_tokens = (0, summarization_Utils_1.estimate_tokens)(context_text);
    let complete_ai_message = '';
    let has_content = false;
    // Get fallback models for automatic fallback
    const fallback_models = (0, ai_Providers_1.get_recommended_fallbacks)(normalized_model_name);
    const result = await (0, ai_Providers_1.stream_text_with_fallback)({
        options: {
            modelName: normalized_model_name,
...
exports.handle_chat_message = handle_chat_message;
